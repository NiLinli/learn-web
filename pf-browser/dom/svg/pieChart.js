function pieChart(data, width, height, cx, cy, r, colors, labels, lx, ly) {
  ...
  var chart = document.createElementNS(svgns, "svg:svg"); // 创建一个有 namespace 的 DOM 节点
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    // These formulas are chosen so that an angle of 0 is at 12 o'clock // and positive angles increase clockwise.
    var x1 = cx + r * Math.sin(startangle);
    var y1 = cy - r * Math.cos(startangle);
    var x2 = cx + r * Math.sin(endangle);
    var y2 = cy - r * Math.cos(endangle);

    // This is a flag for angles larger than than a half circle // It is required by the SVG arc drawing component
    var big = 0;
    ...
    // The next wedge begins where this one 
    startangle = endangle;
